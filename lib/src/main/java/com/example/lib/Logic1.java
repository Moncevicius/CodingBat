...
public class Logic1 {
     ...
     * the result, encoded as an int value: 0=no ticket, 1=small ticket, 2=big ticket. If
     * speed is 60 or less, the result is 0. If speed is between 61 and 80 inclusive, the result
     ...
     * multiple of 11. Return true if the given non-negative number is special. Use the %
    ...
    public boolean specialElevent(int n) {
     ...
     * are at least 5. However, if either tea or candy is at least double the amount of
     * the other one, the party is great (2). However, in all cases, if either tea or
     ...
     * 2 5 11, or 5 6 7, but not 6 5 7 or 5 5 7. However, with the exception that if
    ...
    public boolean lastDigit(int a, int b, int c) {
        ...
        return (b % 10 == c % 10) || (a % 10 == c % 10) || (a % 10 == b % 10);
     ...
     * if noDoubles is true, if the two dice show the same value, increment one die to the
    ...
    public int sumLimit(int a, int b) {
        ...
        String testA = String.valueOf(a);
        String testB = String.valueOf(a + b);

        if (testA.length() == testB.length()) return a + b;
        return a;
